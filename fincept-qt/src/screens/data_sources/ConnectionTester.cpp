...
void test_connection(QWidget* parent, const QString& conn_id, const TestResultCallback& on_result) {
    ...
    const auto test_future = QtConcurrent::run([parent_guard, captured_conn_id, captured_display, captured_test_url,
                                                captured_host, captured_port, captured_cb]() {
        bool success = false;
        ...
        QMetaObject::invokeMethod(
            ...
            [parent_guard, captured_conn_id, captured_display, success, message, captured_cb]() {
                if (!parent_guard)
                    return;
                LOG_INFO(TAG, QString("Test %1: %2 — %3").arg(captured_display, success ? "OK" : "FAIL", message));
                if (captured_cb)
                    captured_cb(captured_conn_id, success, message);
