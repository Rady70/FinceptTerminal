...
std::vector<ToolDef> get_news_tools() {
    ...
    // ── get_news_clusters ───────────────────────────────────────────────
    //
    // Groups related stories across sources to distinguish a widely covered
    // event from a one-off post. Each cluster carries a lead article + sibling articles, source
