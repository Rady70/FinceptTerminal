...
static pr::PredictionMarket parse_market(const QJsonObject& obj) {
    ...
    m.key.asset_ids = {m.key.market_id + QStringLiteral(":yes"), m.key.market_id + QStringLiteral(":no")};
    ...
    if (m.question.isEmpty())
        m.question = obj.value("yes_sub_title").toString();
    ...
    m.extras.insert(QStringLiteral("settlement_value_dollars"), str_or_num(obj.value("settlement_value_dollars")));
...
static pr::PredictionEvent parse_event(const QJsonObject& obj) {
    ...
    if (e.title.isEmpty())
        e.title = obj.value("sub_title").toString();
...
void KalshiRestClient::set_demo_mode(bool demo) {
    demo_ = demo;
}

QString KalshiRestClient::base_url() const {
    return demo_ ? kDemoBase : kProdBase;
}
...
void KalshiRestClient::get_json(const QString& path, JsonCallback on_success, const QString& error_ctx) {
    ...
    req.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("MarketLabTerminal/0.1.0"));
    ...
    connect(reply, &QNetworkReply::finished, this, [self, reply, on_success, error_ctx]() {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            const QString msg = QStringLiteral("%1: %2").arg(reply->errorString(), QString::fromUtf8(data.left(200)));
...
void KalshiRestClient::fetch_markets(const QString& status, const QString& event_ticker, const QString& series_ticker,
                                     const QString& tickers, int limit, const QString& cursor) {
    QUrlQuery q;
    if (!status.isEmpty())
        q.addQueryItem("status", status);
    if (!event_ticker.isEmpty())
        q.addQueryItem("event_ticker", event_ticker);
    if (!series_ticker.isEmpty())
        q.addQueryItem("series_ticker", series_ticker);
    if (!tickers.isEmpty())
        q.addQueryItem("tickers", tickers);
    ...
    QPointer<KalshiRestClient> self = this;
    get_json(
        u.toString(base_url().startsWith(QStringLiteral("https")) ? QUrl::FullyEncoded : QUrl::PrettyDecoded),
...
void KalshiRestClient::fetch_events(const QString& status, const QString& series_ticker, bool with_nested_markets,
    ...
    get_json(
        ...
        [self](const QJsonDocument& doc) {
            ...
            for (const auto& v : arr)
                events.push_back(parse_event(v.toObject()));
...
void KalshiRestClient::fetch_event(const QString& event_ticker) {
    QPointer<KalshiRestClient> self = this;
    get_json(
        QStringLiteral("/events/") + event_ticker + QStringLiteral("?with_nested_markets=true"),
        [self](const QJsonDocument& doc) {
            if (!self)
                return;
            auto obj = doc.object();
...
void KalshiRestClient::fetch_order_book(const QString& ticker, int depth) {
    QUrlQuery q;
    if (depth > 0)
        q.addQueryItem("depth", QString::number(depth));
...
void KalshiRestClient::fetch_candlesticks(const QString& series_ticker, const QString& ticker, int period_interval_min,
                                          qint64 start_ts, qint64 end_ts) {
    QUrlQuery q;
    q.addQueryItem("period_interval", QString::number(period_interval_min));
    if (start_ts > 0)
        q.addQueryItem("start_ts", QString::number(start_ts));
    if (end_ts > 0)
        q.addQueryItem("end_ts", QString::number(end_ts));
    QUrl u(base_url() + QStringLiteral("/series/") + series_ticker + QStringLiteral("/markets/") + ticker +
...
static pr::PriceHistory parse_candlestick_history(const QJsonArray& arr, const QString& asset_id) {
    pr::PriceHistory h;
    h.asset_id = asset_id;
    h.points.reserve(arr.size());
    for (const auto& v : arr) {
...
void KalshiRestClient::fetch_exchange_status() {
    QPointer<KalshiRestClient> self = this;
    get_json(
        QStringLiteral("/exchange/status"),
        [self](const QJsonDocument& doc) {
            if (!self)
                return;
...
void KalshiRestClient::fetch_exchange_schedule() {
    QPointer<KalshiRestClient> self = this;
    get_json(
        QStringLiteral("/exchange/schedule"),
        [self](const QJsonDocument& doc) {
            if (!self)
                return;
...
void KalshiRestClient::fetch_series_detail(const QString& series_ticker) {
    QPointer<KalshiRestClient> self = this;
    get_json(
        QStringLiteral("/series/") + series_ticker,
        [self](const QJsonDocument& doc) {
            if (!self)
                return;
...
void KalshiRestClient::fetch_series_fee_changes() {
    QPointer<KalshiRestClient> self = this;
    get_json(
        QStringLiteral("/series/fee_changes"),
        [self](const QJsonDocument& doc) {
            if (!self)
                return;
            // Response is { "fee_changes": [...] } — emit the array.
            emit self->series_fee_changes_ready(doc.object().value("fee_changes").toArray());
        },
...
void KalshiRestClient::fetch_batch_candlesticks(const QStringList& tickers, int period_interval_min, qint64 start_ts,
                                                qint64 end_ts) {
    if (tickers.isEmpty()) {
    ...
    get_json(
        ...
        [self](const QJsonDocument& doc) {
            ...
            for (const auto& v : arr) {
                ...
                if (t.isEmpty())
                    continue;
                out.insert(t, parse_candlestick_history(o.value("candlesticks").toArray(), t + QStringLiteral(":yes")));
            }
...
void KalshiRestClient::fetch_historical_markets(const QString& series_ticker, int limit, const QString& cursor) {
    QUrlQuery q;
    if (!series_ticker.isEmpty())
        q.addQueryItem("series_ticker", series_ticker);
    q.addQueryItem("limit", QString::number(qBound(1, limit, 1000)));
    ...
    get_json(
        ...
        [self](const QJsonDocument& doc) {
            ...
            markets.reserve(arr.size());
            for (const auto& v : arr)
                markets.push_back(parse_market(v.toObject()));
            emit self->historical_markets_ready(markets, obj.value("cursor").toString());
        },
        QStringLiteral("Kalshi.fetch_historical_markets"));
}

void KalshiRestClient::fetch_historical_candlesticks(const QString& ticker, int period_interval_min, qint64 start_ts,
                                                     qint64 end_ts) {
    QUrlQuery q;
    q.addQueryItem("period_interval", QString::number(period_interval_min));
    if (start_ts > 0)
    ...
    if (end_ts > 0)
        q.addQueryItem("end_ts", QString::number(end_ts));
    QUrl u(base_url() + QStringLiteral("/historical/markets/") + ticker + QStringLiteral("/candlesticks"));
    u.setQuery(q);
    ...
    get_json(
        ...
        [self, ticker_copy](const QJsonDocument& doc) {
            if (!self)
                return;
            auto h = parse_candlestick_history(doc.object().value("candlesticks").toArray(),
...
void KalshiRestClient::fetch_historical_trades(const QString& ticker, int limit, const QString& cursor) {
    QUrlQuery q;
    if (!ticker.isEmpty())
        q.addQueryItem("ticker", ticker);
    q.addQueryItem("limit", QString::number(qBound(1, limit, 1000)));
    ...
    get_json(
        ...
        [self](const QJsonDocument& doc) {
            ...
            trades.reserve(arr.size());
            for (const auto& v : arr)
                trades.push_back(parse_trade(v.toObject()));
...
void KalshiRestClient::fetch_search_tags_by_categories() {
    QPointer<KalshiRestClient> self = this;
    get_json(
        QStringLiteral("/search/tags_by_categories"),
        [self](const QJsonDocument& doc) {
            if (!self)
                return;
