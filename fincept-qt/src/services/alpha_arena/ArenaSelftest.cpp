...
int run_arena_selftest() {
    ...
    ArenaAgentSpec a1;
    a1.provider = "openai";
    a1.model_id = "gpt-test";
    a1.display_name = "GPT";
    a1.source_kind = "provider";
    ArenaAgentSpec a2;
    a2.provider = "anthropic";
    a2.model_id = "claude-test";
    a2.display_name = "Claude";
    a2.source_kind = "provider";
    cfg.agents = {a1, a2};
    ...
    for (int i = 1; i <= 3; ++i) {
        DecisionRow d;
        d.competition_id = cid;
        d.agent_id = aid;
        d.round_seq = i;
        d.status = "ok";
        d.actions_json = QStringLiteral("[%1]").arg(i);
        check("store: insert_decision", store.insert_decision(d).is_ok());
    }
    auto recents = store.recent_decisions(cid, aid, 2);
    check("store: recent_decisions oldest-first window", recents.is_ok() && recents.value().size() == 2 &&
                                                             recents.value()[0].round_seq == 2 &&
                                                             recents.value()[1].round_seq == 3);

    OrderRow o;
    o.competition_id = cid;
    o.agent_id = aid;
    o.round_seq = 1;
    o.coin = "BTC";
    o.side = "long";
    ...
    check("store: orders_for round-trip", orders_r.is_ok() && orders_r.value().size() == 1 &&
                                              orders_r.value()[0].coin == "BTC" &&
                                              approx(orders_r.value()[0].notional_usd, 5000));

    EquitySnapshotRow es;
    es.competition_id = cid;
    es.agent_id = aid;
    es.round_seq = 1;
    es.ts = 1500;
    es.equity = 10100;
    es.balance = 9000;
    es.unrealized_pnl = 100;
    ...
    check("store: recent_events oldest-first", ev_r.is_ok() && ev_r.value().size() == 2 &&
                                                   ev_r.value()[0].contains("test_event") &&
                                                   ev_r.value()[1].contains("test_event_2"));
    ...
    // ── Section 3: decision parser ──────────────────────────────────────────
    {
        ...
        auto p1 = parse_decision(
            R"({"actions":[{"action":"open","coin":"BTC","side":"long","size_usd":2000,"leverage":3,"reason":"r"}]})",
            instr, 10);
        check("parse: clean json", p1.ok() && p1.actions.size() == 1 && p1.actions[0].kind == ActionKind::Open &&
                                       p1.actions[0].coin == "BTC" && approx(p1.actions[0].size_usd, 2000) &&
                                       approx(p1.actions[0].leverage, 3));
        auto p2 = parse_decision(
            "Here is my plan:\n```json\n{\"actions\":[{\"action\":\"hold\",\"reason\":\"wait\"}]}\n```\nthanks", instr,
            10);
        check("parse: fenced json", p2.ok() && p2.actions.size() == 1 && p2.actions[0].kind == ActionKind::Hold);
        auto p3 = parse_decision(
            "I think {\"actions\":[{\"action\":\"close\",\"coin\":\"ETH\",\"reason\":\"tp\"}]} fits", instr, 10);
        check("parse: embedded json",
              p3.ok() && p3.actions[0].kind == ActionKind::Close && p3.actions[0].coin == "ETH");
        ...
        check("parse: unknown coin fails",
              !parse_decision(
                   R"({"actions":[{"action":"open","coin":"DOGE","side":"long","size_usd":100,"leverage":2}]})", instr,
                   10)
                   .ok());
        check("parse: leverage cap fails",
              !parse_decision(
                   R"({"actions":[{"action":"open","coin":"BTC","side":"long","size_usd":100,"leverage":50}]})", instr,
                   10)
                   .ok());
        check("parse: bad side fails",
              !parse_decision(
                   R"({"actions":[{"action":"open","coin":"BTC","side":"sideways","size_usd":100,"leverage":2}]})",
                   instr, 10)
                   .ok());
    ...
    // ── Section 4: paper exchange ───────────────────────────────────────────
    {
        ...
        AgentAction flip = open;
        flip.side = "short";
        flip.size_usd = 1000;
        flip.leverage = 2;
        auto f3 = px.execute(paid, flip, 3);
        auto av4 = px.account(paid);
        check("px: flip leaves single short",
              f3.is_ok() && av4.positions.size() == 1 && av4.positions[0].side == "short");
        ...
        if (auto fo = store.orders_for(pcid, paid, 50); fo.is_ok())
            for (const auto& ord : fo.value())
                if (ord.round_seq == 3 && ord.action == "close" && ord.realized_pnl > 0)
                    flip_pnl = true;
        check("px: flip realized pnl recorded", flip_pnl);

        // Full close.
        AgentAction close;
        close.kind = ActionKind::Close;
        close.coin = "BTC";
    ...
    // ── Section 6: model registry ───────────────────────────────────────────
    {
        ...
        for (const auto& o : opts) {
            ...
            if (keys.contains(k))
                dup = true;
            keys.insert(k);
            if (o.color_hex.isEmpty() || o.display_name.isEmpty() || o.source_kind.isEmpty())
                blank = true;
        }
    ...
    {
        CompetitionRow comp;
        comp.capital_per_agent = 10000;
        comp.instruments = {"BTC", "ETH"};
        comp.max_leverage = 10;
        comp.cadence_seconds = 180;
    ...
    {
        ArenaLlmRequest rq;
        rq.provider = "anthropic";
        rq.model_id = "claude-x";
        rq.system_prompt = "S";
        rq.user_prompt = "U";
        rq.max_tokens = 1000;
        ...
        auto pr1 = ArenaLlmClient::parse_response(
            "openai",
        ...
        auto pr2 = ArenaLlmClient::parse_response(
            "anthropic", R"({"content":[{"type":"text","text":"yo"}],"usage":{"input_tokens":7,"output_tokens":3}})");
        check("llm: anthropic parse", pr2.success && pr2.content == "yo" && pr2.completion_tokens == 3);
        auto pr3 = ArenaLlmClient::parse_response(
            "gemini",
        ...
        check("llm: endpoint composition",
              fincept::ai_chat::ProviderCatalog::chat_endpoint("openai", "", "m") ==
                      "https://api.openai.com/v1/chat/completions" &&
                  fincept::ai_chat::ProviderCatalog::chat_endpoint("deepseek", "https://my.proxy/v1", "m") ==
                      "https://my.proxy/v1/chat/completions" &&
                  fincept::ai_chat::ProviderCatalog::chat_endpoint("anthropic", "https://gw.corp", "m") ==
                      "https://gw.corp/v1/messages");
    }
    ...
    // ── Section 9: market data parsers ──────────────────────────────────────
    {
        ...
        snap.coins["BTC"] = b;
        const auto ctx_doc =
            QJsonDocument::fromJson(R"([{"universe":[{"name":"BTC"},{"name":"ETH"}]},)"
        ...
        check("md: ctxs applied by index",
              approx(snap.coins["BTC"].funding_rate, 0.0000125, 1e-9) &&
        ...
        check("md: candles parsed",
              candles.size() == 1 && approx(candles[0].c, 93858.0) && candles[0].t == 1765299600000LL);
    }
    ...
    // ── Section 10: engine round (fakes; no network) ────────────────────────
    {
        ...
        fake_llm->scripts["gpt-test"].enqueue(
            R"({"actions":[{"action":"open","coin":"BTC","side":"long","size_usd":2000,"leverage":2,"reason":"momentum"}]})");
        ...
        g.model_id = "gpt-test";
        g.display_name = "GPT";
        g.source_kind = "provider";
        g.source_ref = "ollama";
        ArenaAgentSpec c = g;
        c.model_id = "claude-test";
        c.display_name = "Claude";
        ecfg.agents = {g, c};

        const QString ecid = ArenaEngine::instance().create_competition(ecfg).value();
        QEventLoop loop;
        int completed_seq = 0;
        QObject::connect(&eng, &ArenaEngine::round_completed, &loop, [&](int s) {
            completed_seq = s;
            loop.quit();
        });
        ...
        for (const auto& a : agents_now)
            (a.model_id == "gpt-test" ? gpt_id : claude_id) = a.id;
        auto orders = store.orders_for(ecid, gpt_id, 10);
        check("engine: gpt order filled", orders.is_ok() && orders.value().size() == 1 &&
                                              orders.value()[0].status == "filled" &&
                                              approx(orders.value()[0].notional_usd, 2000));
        ...
        check("engine: parse failure did not block round", eq.is_ok() && eq.value().size() == 2 && completed_seq == 1);
        ...
        QObject::connect(&eng, &ArenaEngine::hitl_requested, &loop, [&](QString id, QString aid, QString) {
            approval_id = id;
            hitl_agent = aid;
        });
        int hseq = 0;
        ...
        QTimer::singleShot(5000, &loop,
                           &QEventLoop::quit); // watchdog (round_completed quits via the Section-10 connect)
        loop.exec();
        ...
        check("hitl: no order before approval", store.orders_for(hcid, hagent, 10).value().isEmpty());
        ...
        check("hitl: order executed after approval",
              horders.is_ok() && horders.value().size() == 1 && horders.value()[0].status == "filled");
        (void)eng.kill_all();
        ...
        if (auto ev = store.recent_events(kcid, 20); ev.is_ok())
            for (const auto& e : ev.value())
                if (e.contains("actions_skipped_agent_halted"))
                    halt_event = true;
        ...
        check("kill: setup round opened a position", completed_seq == 2 && kpos.is_ok() && kpos.value().size() == 1);
        ...
        if (auto ko = store.orders_for(kcid, kaid, 50); ko.is_ok())
            for (const auto& o : ko.value())
                if (o.action == "close" && o.status == "filled")
                    kill_close = true;
