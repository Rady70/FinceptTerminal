...
QString RealYieldService::resolve_endpoint() const {
    ...
    if (r.is_ok())
        return trim_trailing_slash_yield(r.value().trimmed());
    return {};
}
...
void RealYieldService::refresh(const QStringList& topics) {
    ...
    if (mock) {
        ...
        if (!once.exchange(true)) {
            LOG_INFO("RealYield", "no fincept.yield_endpoint configured — serving mock yield");
        } else {
    ...
    for (const auto& topic : topics) {
        if (topic.startsWith(QLatin1String(kYieldFamily))) {
            ...
            if (pk.isEmpty())
                continue;
            mock ? publish_mock_yield(topic, pk) : refresh_yield_real(endpoint, topic, pk);
...
void RealYieldService::refresh_yield_real(const QString& endpoint, const QString& topic, const QString& pubkey) {
    ...
    QObject::connect(reply, &QNetworkReply::finished, this, [self, reply, topic, pubkey]() {
        ...
        s.last_period_end_ts = static_cast<qint64>(j.value(QStringLiteral("last_period_end_ts")).toDouble());
...
void RealYieldService::refresh_revenue_real(const QString& endpoint) {
    ...
    QObject::connect(reply, &QNetworkReply::finished, this, [self, reply]() {
        ...
        const auto doc = QJsonDocument::fromJson(reply->readAll());
        if (!doc.isObject()) {
            hub.publish_error(QString::fromLatin1(kTopicRevenue), QStringLiteral("malformed JSON"));
            return;
        }
        const auto j = doc.object();
        TreasuryRevenue r;
        r.period_start_ts = static_cast<qint64>(j.value(QStringLiteral("period_start_ts")).toDouble());
        r.period_end_ts = static_cast<qint64>(j.value(QStringLiteral("period_end_ts")).toDouble());
...
void RealYieldService::publish_mock_revenue() {
    ...
    r.ts_ms = now;
    r.is_mock = true;
    fincept::datahub::DataHub::instance().publish(QString::fromLatin1(kTopicRevenue), QVariant::fromValue(r));
}
