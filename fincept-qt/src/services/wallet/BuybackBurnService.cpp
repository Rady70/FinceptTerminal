...
void BuybackBurnService::refresh(const QStringList& topics) {
    ...
    if (mock) {
        ...
        if (!once.exchange(true)) {
            LOG_INFO("BuybackBurn", "no fincept.treasury_endpoint configured — serving mock data");
        } else {
...
void BuybackBurnService::refresh_buyback_epoch(const QString& endpoint) {
    ...
    req.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("MarketLabTerminal/BuybackBurnService"));
    ...
    QObject::connect(reply, &QNetworkReply::finished, this, [self, reply]() {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            hub.publish_error(QString::fromLatin1(kTopicBuybackEpoch), reply->errorString());
            return;
        }
        const auto doc = QJsonDocument::fromJson(reply->readAll());
        if (!doc.isObject()) {
            hub.publish_error(QString::fromLatin1(kTopicBuybackEpoch), QStringLiteral("malformed JSON"));
            return;
        }
        ...
        e.start_ts_ms = static_cast<qint64>(j.value(QStringLiteral("start_ts_ms")).toDouble());
        e.end_ts_ms = static_cast<qint64>(j.value(QStringLiteral("end_ts_ms")).toDouble());
...
void BuybackBurnService::refresh_burn_total(const QString& endpoint) {
    ...
    QObject::connect(reply, &QNetworkReply::finished, this, [self, reply]() {
        ...
        auto& hub = fincept::datahub::DataHub::instance();
        if (reply->error() != QNetworkReply::NoError) {
            hub.publish_error(QString::fromLatin1(kTopicBurnTotal), reply->errorString());
            return;
        }
        const auto doc = QJsonDocument::fromJson(reply->readAll());
        if (!doc.isObject()) {
            hub.publish_error(QString::fromLatin1(kTopicBurnTotal), QStringLiteral("malformed JSON"));
            return;
        }
        const auto j = doc.object();
...
void BuybackBurnService::refresh_supply_history(const QString& endpoint) {
    ...
    QObject::connect(reply, &QNetworkReply::finished, this, [self, reply]() {
        ...
        auto& hub = fincept::datahub::DataHub::instance();
        if (reply->error() != QNetworkReply::NoError) {
            hub.publish_error(QString::fromLatin1(kTopicSupplyHistory), reply->errorString());
            return;
        }
        const auto doc = QJsonDocument::fromJson(reply->readAll());
        if (!doc.isObject()) {
            hub.publish_error(QString::fromLatin1(kTopicSupplyHistory), QStringLiteral("malformed JSON"));
            return;
        }
...
void BuybackBurnService::publish_mock_burn_total() {
    ...
    t.ts_ms = QDateTime::currentMSecsSinceEpoch();
    t.is_mock = true;
    fincept::datahub::DataHub::instance().publish(QString::fromLatin1(kTopicBurnTotal), QVariant::fromValue(t));
}
...
void BuybackBurnService::publish_mock_supply_history() {
    ...
    for (int i = 11; i >= 0; --i) {
        ...
        const qint64 burned = kStartBurn + static_cast<qint64>((kEndBurn - kStartBurn) * t);
